...
for (i in c(1:10)){
  for (j in c(2:11)){
    ...
    cash_flow = c(-dirty_price, rep(cp/2, length(time_point)-2), 100)
    # Calculate the YTM
...
for (i in c(1:10)){
  ...
  spot_rate = -log(dirty_price/100)/bond_prices$time[1]
  spot[i, 1] = spot_rate
}
...
for (i in c(1:10)){
  for (j in c(2:11)){
    ...
    accrued = 0.25 * cp
    dirty_price = price + accrued
    spot_rate = -log((dirty_price - sum((cp/2)*exp(-r*t)))/100)/bond_prices$time[j]
    spot[i, j] = spot_rate
  }
}
